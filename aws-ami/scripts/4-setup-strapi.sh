#!/bin/bash -e

# load temp password from database script
PASS=$(cat ~/temp_pgpass)

# install strapi with PostgreSQL
echo "Creating the Strapi application"
cd /srv/strapi; sudo yarn create strapi-app strapi \
--dbclient=postgres \
--dbhost="127.0.0.1" \
--dbport=5432 \
--dbname="strapi" \
--dbusername="strapi" \
--dbpassword="$PASS" \
--dbforce

# build the adminUI
echo "Building the AdminUI"
cd /srv/strapi/strapi; sudo yarn build > /dev/null

# move config files
echo "Moving Strapi config files and scripts"
sudo mv /tmp/strapi/index.html /srv/strapi/strapi/public/
sudo mv /tmp/strapi/server.json /srv/strapi/strapi/config/environments/development/
sudo mv /tmp/strapi/set-strapi-ip.sh /var/lib/cloud/scripts/per-instance/
sudo chmod +x /var/lib/cloud/scripts/per-instance/set-strapi-ip.sh

# ensure the strapi user owns the data dir
echo "Ensuring Strapi owns all needed files and directories"
sudo chown -R strapi:strapi /srv/strapi

# spinup the server and set to run on boot
echo "Starting Strapi to generate any automated files"
sudo su - strapi -c "cd /srv/strapi/strapi && pm2 start npm --name strapi-develop -- run develop > /dev/null"
sleep 30

# run strapi on boot
echo "Setting Strapi to run on boot"
sudo env PATH=$PATH:/usr/bin /usr/local/share/.config/yarn/global/node_modules/pm2/bin/pm2 startup systemd -u strapi --hp /srv/strapi > /dev/null
sudo su - strapi -c "pm2 save > /dev/null"

# cleanup temp_pgpass
rm ~/temp_pgpass